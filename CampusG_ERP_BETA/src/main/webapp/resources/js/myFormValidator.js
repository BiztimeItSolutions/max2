$(document).ready(function(){
	...
	/*$("#saveMe").click(function(event) {
		...
		if ($("#admissionForm").valid()) {
			 var message="<button class='close' aria-hidden='true' data-dismiss='alert' type='button'>×</button>"+"The submitted form has the mandatory data that has to be filled, please re-check";
	...
	*/
	//country state
				...
				s_a[105] = "Select State|Andaman and Nicobar Islands|Andhra Pradesh|Arunachal Pradesh|Assam|Bihar|Chandigarh|Chhattisgarh|Dadra and Nagar Haveli|Daman and Diu|Delhi|Goa|Gujarat|Haryana|Himachal Pradesh|Jammu and Kashmir|Jharkhand|Karnataka|Kerala|Lakshadweep|Madhya Pradesh|Maharashtra|Manipur|Meghalaya|Mizoram|Nagaland|Orissa|Pondicherry|Punjab|Rajasthan|Sikkim|Tamil Nadu|Tripura|Uttar Pradesh|Uttaranchal|West Bengal";
				...
				var country_arr = new Array("Select Country","Afghanistan", "Albania",
						"Algeria", "American Samoa", "Angola", "Anguilla",
						"Antartica", "Antigua and Barbuda", "Argentina",
						...
						"Czeck Republic", "Denmark", "Djibouti", "Dominica",
						...
						"Gambia, The", "Gaza Strip", "Georgia", "Germany",
						...
						"Ireland, Northern", "Israel", "Italy", "Jamaica",
						"Jan Mayen", "Japan", "Jarvis Island", "Jersey",
						"Johnston Atoll", "Jordan", "Juan de Nova Island",
						...
						"Man, Isle of", "Marshall Islands", "Martinique",
						"Mauritania", "Mauritius", "Mayotte", "Mexico",
						"Micronesia, Federated States of", "Midway Islands",
						...
						"Pitcaim Islands", "Poland", "Portugal", "Puerto Rico",
						"Qatar", "Reunion", "Romainia", "Russia", "Rwanda",
						...
						"Svalbard", "Swaziland", "Sweden", "Switzerland",
						"Syria", "Taiwan", "Tajikistan", "Tanzania",
						"Thailand", "Tobago", "Toga", "Tokelau", "Tonga",
						...
						"Wales", "Wallis and Futuna", "West Bank",
						"Western Sahara", "Yemen", "Yugoslavia", "Zambia",
						"Zimbabwe");

				$.each(country_arr, function(i, item) {
					$('#presentaddresscountry').append($('<option>', { 
				        value: i,
				        text : item 
				    }));
				});
				$("#presentaddresscountry").val(105);
				var state_arr = s_a[105].split("|");
				$.each(state_arr, function(i, item) {
					$('#presentaddressstate').append($('<option>', { 
				        value: i,
				        text : item 
				    }));
				});
				$("#presentaddresscountry").change(function() {
					$("#presentaddressstate").empty();
					var state_arr = s_a[$(this).val()].split("|");
					$.each(state_arr, function(i, item) {
						$('#presentaddressstate').append($('<option>', { 
					        value: i,
					        text : item 
	...
	//end of present address country state
				
	//permanent address country state
				$.each(country_arr, function(i, item) {
					$('#permanentaddresscountry').append($('<option>', { 
				        value: i,
				        text : item 
				    }));
				});
				$("#permanentaddresscountry").val(105);
				var state_arr = s_a[105].split("|");
				$.each(state_arr, function(i, item) {
					$('#permanentaddressstate').append($('<option>', { 
				        value: i,
				        text : item 
				    }));
				});
				$("#permanentaddresscountry").change(function() {
					$("#permanentaddressstate").empty();
					var state_arr = s_a[$(this).val()].split("|");
					$.each(state_arr, function(i, item) {
						$('#permanentaddressstate').append($('<option>', { 
					        value: i,
	...
	//end of permanent address country state

	//emergency address country state
				$.each(country_arr, function(i, item) {
					$('#emergencyaddresscountry').append($('<option>', { 
				        value: i,
				        text : item 
				    }));
				});
				$("#emergencyaddresscountry").val(105);
				var state_arr = s_a[105].split("|");
				$.each(state_arr, function(i, item) {
					$('#emergencyaddressstate').append($('<option>', { 
				        value: i,
				        text : item 
				    }));
				});
				$("#emergencyaddresscountry").change(function() {
					$("#emergencyaddressstate").empty();
					var state_arr = s_a[$(this).val()].split("|");
					$.each(state_arr, function(i, item) {
						$('#emergencyaddressstate').append($('<option>', { 
					        value: i,
	...
	//end of emergency address country state			

	
	//end of country state
 ...
 * 
 function parseDate()
{
	alert("parse date called");
//			var data=$("#datepicker").val();
//			alert("button clicked: "+data);
//			
//			var splitData=data.split("-");
//			alert("split: "+splitData);
//			var sqlDate=splitData.reverse().join("-");
//			alert("reverse: "+sqlDate);
//			$("#sqlDate").val(sqlDate);
//			$("#newDateForm").submit();

}

function findError(rel){
	
//	"<button class='close' aria-hidden='true' data-dismiss='alert' type='button'>×</button>"+
	if(rel==1){
		var msg="you have error in"+"<br>";
		var firstTabData={firstname:$("#firstname").val(),dob:$("#dob").val(),nationality:$("#nationality").val()};
		var flag=1;
		$.each(firstTabData,function(id,val){
			
			alert("value: "+val);
			if(val==""){
				flag=0;
				$("#divErr").hide();
				msg=msg +":"+$("#"+id).parents("div.form-group").find("label").text();
				 $("#divErr").html(msg);
				 $("#divErr").show();
	...
	//"<button class='close' aria-hidden='true' data-dismiss='alert' type='button'>×</button>"+
	if(rel==2){
		var msg="Please fill the following required details"+"<br>";
		
		var secondTabData={presentaddressline1:$("#presentaddressline1").val(),presentaddresscity:$("#presentaddresscity").val(),presentaddresspincode:$("#presentaddresspincode").val(),presentaddressstate:$("#presentaddressstate").val(),
			permanentaddressline1:$("#permanentaddressline1").val(),permanentaddresscity:$("#permanentaddresscity").val(),permanentaddresspincode:$("#permanentaddresspincode").val(),permanentaddressstate:$("#permanentaddressstate").val(),
			emergencyaddressline1:$("#emergencyaddressline1").val(),emergencyaddresscity:$("#emergencyaddresscity").val(),emergencyaddresspincode:$("#emergencyaddresspincode").val(),emergencyaddressstate:$("#emergencyaddressstate").val()};
		var flag=1;
		$.each(secondTabData,function(id,val){
			
			if(val==""){
				
				flag=0;
				$("#divErr").hide();
				msg=msg+":"+$("#"+id).parents("div.form-group").find("label").text()+" in "+$("#"+id).parents("div.col-lg-5").find("b").text()+"<br/>"+"&nbsp";
